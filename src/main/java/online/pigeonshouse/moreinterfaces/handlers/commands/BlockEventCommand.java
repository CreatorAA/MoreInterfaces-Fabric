    ...
    public static boolean activateListeners(ChatSession session, String event, CommandMap map) {
        ...
        switch (event) {
            case "block_state" -> {
                ...
                ServerLevel level = pos.getWorld().isBlank() ? server.overworld() :
                ...
                BlockStateEventHandler handler = new BlockStateEventHandler(session, pos, level);
            ...
            case "area_entities_info" -> {
                ...
                ServerLevel level = WorldUtil.getLevel(server.overworld(),
                        pos1.getWorld());
                if (level == null) {
                ...
                if (width * height > maxArea) {
                    session.sendErrorMsg(MessageCode.AREA_TOO_LARGE, "区域总面积不应该超过：%s, 您选择的面积：%s"
                            .formatted(maxArea, area));
                    return false;
                }
                ...
                AreaScanTask task = new AreaScanTask(session, miArea, level);
